...
for ano in range(2015, 2025):
    ...
    for it in items:
        ...
        if is_liquidada_col(coluna) and valor > 0:
            if cod_conta == "RREO2TotalDespesas" or cod_conta == "":
                ...
                if "seguranca publica" in conta_norm:
                    total_f06 = max(total_f06, valor)
                elif any(sub in conta_norm for sub in ["policiamento", "defesa civil", "inteligencia", "administracao geral", "demais subfuncoes", "fu06"]):
    ...
    for sub, val in subfuncs.items():
        records.append({
            ...
            "Participação (%)": round((val / total_f06) * 100, 2) if total_f06 > 0 else 0.0
        })

df = pd.DataFrame(records)
# Display sorted by Ano and Valor
